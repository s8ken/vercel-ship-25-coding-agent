// V0 ↔ Blackbox Adapter - Clean implementation
import { getAgentConfig } from "./model-routing"
...
export function baseHeaders({
  ...
  const h: Record<string, string> = {
    ...
    "X-SU-Remaining": "TBD",
    "X-SU-Percent": "TBD",
    "X-Resets-At": new Date(Date.now() + 1000 * 60 * 60 * 24 * 30).toISOString(),
  }
  return h
}

export function withHeaders(res: Response, rid: string, suRemain: number, suPct: number, tier: string) {
  const h = new Headers(res.headers)
  h.set("symbi-request-id", rid)
  h.set("X-Plan", tier)
  h.set("X-SU-Remaining", String(suRemain))
  h.set("X-SU-Percent", String(suPct))
